          ...
          <li key={c.id} className="border p-4 rounded shadow">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p>{c.description}</p>
                {c.imageUrl && (
                  <img src={c.imageUrl} alt={c.name} className="mt-2 h-24 rounded" />
                )}
              </div>
              <div className="space-x-2">
                <button onClick={() => startEdit(c)} className="text-yellow-600">
                  Edit
                </button>
                <button onClick={() => dispatch(deleteCourseStart(c.id))} className="text-red-600">
                  Delete
                </button>
              </div>
            </div>
